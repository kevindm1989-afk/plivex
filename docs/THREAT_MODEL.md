...
This document describes what Plivex is and is not designed to protect against. It's a companion to `PRIVACY.md` and `SECURITY.md`, and the explicit reference for what's in scope when evaluating a security claim or a vulnerability report.

This is not a contract. It does not create obligations on the developer beyond what `LICENSE` and `TERMS.md` already establish. It describes the security properties of the code as designed, in plain terms.
...
The following are explicitly out of scope. Plivex provides no defense:
...
2. **Browser extensions.** Browser extensions installed by the user have access to the page's DOM and JavaScript context. A malicious extension can read decrypted entries, capture passphrases, or modify entries before they're hashed.
...
6. **Forgotten passphrases.** By design, the developer cannot recover data when the passphrase is forgotten. The "I forgot my passphrase" path is a wipe.
...
8. **OS-level forensics on an unlocked device.** Browser swap files, browser history, paged-out memory, OS-level "remember last activity" features, and similar artifacts may contain plaintext fragments. Plivex cannot prevent the operating system from caching its own data.
...
14. **Adversaries with access to backup files outside the device.** The export file is encrypted (entries remain encrypted), but it includes the wrapped master key. Anyone with the export file plus the passphrase can decrypt all entries. Treat exports as sensitive — store them only in places the user trusts.

15. **Adversaries with multiple historical snapshots.** The hash chain protects against tampering with the data the user wrote, but if an adversary obtains a database snapshot at time T1 and another at T2, they can detect that data changed between T1 and T2 even if both states are internally consistent. Plivex does not provide forward secrecy.

16. **Compelled disclosure.** Legal process compelling the user to provide their passphrase is outside the cryptographic model. Plivex does not provide a defense against authorities that can compel disclosure of a passphrase.
...
| Browser extensions | **NO** | Treated as part of the page's threat model; user must avoid untrusted extensions |
| GitHub Pages CDN at first load | trust-then-verify | Cached locally after first install; users concerned can self-host |
...
- Avoid installing untrusted browser extensions. Audit existing ones.
- Don't share or write down the passphrase.
- Export backups regularly to a location the user controls. Don't email exports to yourself or upload them to a service that can read your encrypted-at-rest backups.
- For high-stakes use, self-host: fork the repo, deploy to your own GitHub Pages or local file server, so the trust boundary doesn't include the original publisher's GitHub account.
- Treat exports as sensitive — they include the wrapped master key, so the export plus the passphrase decrypts everything.
- Periodically run **Settings → Verify integrity** to catch tampering.
- Set the auto-lock timeout to a value appropriate to the threat: smaller for shared devices, larger for personal ones.

## What this document is not

This document describes the security properties of the code as designed. It does not promise that the code is free of bugs. It does not create a contractual obligation. The software is provided "as-is" per `LICENSE` and `TERMS.md`. If you find a vulnerability, see `SECURITY.md` for the private-disclosure path.
